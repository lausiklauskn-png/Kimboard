/*
 * Kimboard — Siegel-Modal-Inhalt (das Andock-Werkzeug IM Siegel).
 *
...
(function () {
  ...
  function buildWizardDialog() {
    ...
    dlg.innerHTML =
      ...
      '<ol style="padding-left:1.1rem;line-height:1.5;font-size:.9rem">' +
        '<li><b>Identität erzeugen</b> — Ed25519-Schlüsselpaar, nodeId aus dem Public Key.<br>' +
          '<button type="button" id="kbdwiz-s1" style="margin:.4em 0;cursor:pointer;border-radius:8px;border:1px solid #C9A961;background:rgba(201,169,97,0.12);color:#F5E6B8;padding:.3em .7em;font:inherit">Identität erzeugen</button>' +
          '<div id="kbdwiz-o1" style="font-family:monospace;font-size:.8rem;color:#6ee7d3;word-break:break-all"></div></li>' +
        '<li style="margin-top:.6em"><b>Spore signieren + herunterladen</b> — mit echtem 384-dim domainVector.<br>' +
          '<button type="button" id="kbdwiz-s2" disabled style="margin:.4em 0;cursor:pointer;border-radius:8px;border:1px solid #C9A961;background:rgba(201,169,97,0.12);color:#F5E6B8;padding:.3em .7em;font:inherit">Spore erzeugen + ⬇</button>' +
          '<div id="kbdwiz-o2" style="font-family:monospace;font-size:.8rem;color:#6ee7d3;word-break:break-all"></div></li>' +
        '<li style="margin-top:.6em"><b>Verschlüsseltes Backup</b> — Passwort-Sicherung (AES-256-GCM/PBKDF2 600k) gegen IndexedDB-Verlust.<br>' +
          '<button type="button" id="kbdwiz-s3" disabled style="margin:.4em 0;cursor:pointer;border-radius:8px;border:1px solid #C9A961;background:rgba(201,169,97,0.12);color:#F5E6B8;padding:.3em .7em;font:inherit">Backup erzeugen + ⬇</button>' +
          '<div id="kbdwiz-o3" style="font-family:monospace;font-size:.8rem;color:#6ee7d3;word-break:break-all"></div></li>' +
    ...
    dlg.querySelector("#kbdwiz-s1").addEventListener("click", function () {
      var b = dlg.querySelector("#kbdwiz-s1");
      if (!window.SbkimSpore || !window.SbkimSpore.getOrCreateIdentity) { out("#kbdwiz-o1", "Modul 02 nicht geladen.", true); return; }
      b.disabled = true; out("#kbdwiz-o1", "Erzeuge Identität …");
      window.SbkimSpore.getOrCreateIdentity().then(function (id) {
        out("#kbdwiz-o1", "nodeId: " + id.nodeId); dlg.querySelector("#kbdwiz-s2").disabled = false;
      ...
      }).catch(function (e) { out("#kbdwiz-o1", "Fehler: " + (e && e.message || e), true); b.disabled = false; });
    });
    dlg.querySelector("#kbdwiz-s2").addEventListener("click", function () {
      var b = dlg.querySelector("#kbdwiz-s2");
      if (!window.SbkimEmbedding || !window.SbkimSpore) { out("#kbdwiz-o2", "Modul 02/03 nicht geladen.", true); return; }
      b.disabled = true;
      ...
      var onProg = function (ev) {
        ...
        if (typeof d.progress === "number" && isFinite(d.progress)) {
          ...
          out("#kbdwiz-o2", "Modell laedt  " + "█".repeat(filled) + "░".repeat(20 - filled) + "  " + pct + " %  (~30 MB einmalig)");
        } else if (d.status === "done" || d.status === "ready") {
          out("#kbdwiz-o2", "Modell geladen ✓");
        }
      };
      window.addEventListener("sbkim:embedding-progress", onProg);
      out("#kbdwiz-o2", "Lade Embedding-Modell (~30 MB, einmalig) …");
      window.SbkimEmbedding.init()
        .then(function () { out("#kbdwiz-o2", "Erzeuge domainVector (384) …"); return window.SbkimEmbedding.embedPassage(WIZ.domainDescription + ". " + WIZ.domainKeywords.join(", ")); })
        .then(function (vec) {
          var arr = Array.from(vec);
        ...
        .then(function (spore) { lastSpore = spore; downloadJson(sporeFileName(), spore);
          out("#kbdwiz-o2", "Spore erzeugt + ⬇ (nodeId=" + spore.id + "). Nach sbkim/spore.json committen.");
          dlg.querySelector("#kbdwiz-s3").disabled = false;
          ...
          var o3 = dlg.querySelector("#kbdwiz-o3");
          if (o3 && /Keine Identit/.test(o3.textContent || "")) o3.textContent = "";
        })
        .catch(function (e) { out("#kbdwiz-o2", "Fehler: " + (e && e.message || e), true); b.disabled = false; })
        .then(function () { window.removeEventListener("sbkim:embedding-progress", onProg); });
    });
    dlg.querySelector("#kbdwiz-s3").addEventListener("click", function () {
      if (!window.SbkimSpore || !window.SbkimSpore.exportBackup) { out("#kbdwiz-o3", "Modul 02 exportBackup fehlt.", true); return; }
      var pw = window.prompt("Backup-Passwort (mind. 8 Zeichen, KEIN Reset möglich):");
      if (!pw) { out("#kbdwiz-o3", "Abgebrochen — kein Passwort.", true); return; }
      var b = dlg.querySelector("#kbdwiz-s3"); b.disabled = true;
      out("#kbdwiz-o3", "Erzeuge Backup (PBKDF2 600k + AES-GCM-256) …");
      window.SbkimSpore.exportBackup(pw).then(function (blob) {
        downloadJson("kimboard-backup-" + new Date().toISOString().replace(/[:.]/g, "-") + ".sbkim.json", blob);
        out("#kbdwiz-o3", "Backup ⬇ — Datei + Passwort sicher aufbewahren.");
      }).catch(function (e) { out("#kbdwiz-o3", "Fehler: " + (e && e.message || e), true); b.disabled = false; });
    });
    dlg.querySelector("#kbdwiz-s4").addEventListener("click", function () { dlg.querySelector("#kbdwiz-s4-file").click(); });
    dlg.querySelector("#kbdwiz-s4-file").addEventListener("change", function (ev) {
      var input = ev.target; var file = input.files && input.files[0];
      if (!window.SbkimSpore || !window.SbkimSpore.importBackup) { out("#kbdwiz-o4", "Modul 02 importBackup fehlt.", true); return; }
      if (!file) { out("#kbdwiz-o4", "Keine Datei gewählt.", true); return; }
      file.text().then(function (text) {
        var blob; try { blob = JSON.parse(text); } catch (e) { out("#kbdwiz-o4", "Datei ist kein gültiges JSON-Backup.", true); input.value = ""; return; }
        var pw = window.prompt("Backup-Passwort eingeben (das beim Sichern vergebene):");
        if (!pw) { out("#kbdwiz-o4", "Abgebrochen — kein Passwort.", true); input.value = ""; return; }
        out("#kbdwiz-o4", "Entschlüssele Backup + spiele Identität zurück …");
        window.SbkimSpore.importBackup(blob, pw).then(afterRestore).catch(function (err) {
          ...
          if (/Overwrite/i.test(name) || /vorhanden|überschreib|overwrite/i.test(msg)) {
            if (window.confirm("Eine Identität existiert bereits im Browser. Mit der Backup-Version überschreiben? (Die jetzige lokale Identität geht verloren.)")) {
              out("#kbdwiz-o4", "Überschreibe vorhandene Identität …");
              window.SbkimSpore.importBackup(blob, pw, { force: true }).then(afterRestore).catch(function (e2) { out("#kbdwiz-o4", "Fehler beim Überschreiben: " + (e2 && e2.message || e2), true); });
            } else { out("#kbdwiz-o4", "Abgebrochen — vorhandene Identität unverändert.", true); }
          } else { out("#kbdwiz-o4", "Fehler: " + msg + " (falsches Passwort oder beschädigte Datei?)", true); }
        }).then(function () { input.value = ""; });
      });
    });
    function afterRestore(res) {
      if (res && res.restored) {
        out("#kbdwiz-o4", "Identität wiederhergestellt — Schlüssel + Spore zurück in der Browser-IndexedDB.");
    ...
    if (window.SbkimSpore && typeof window.SbkimSpore.listIdentities === "function") {
      window.SbkimSpore.listIdentities().then(function (ids) {
        if (ids && ids.length) {
          var s2 = dlg.querySelector("#kbdwiz-s2"); if (s2) s2.disabled = false;
          var s3 = dlg.querySelector("#kbdwiz-s3"); if (s3) s3.disabled = false;
        }
      }).catch(function () {});
    }
  }
