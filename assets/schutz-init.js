/*
 * Kimboard — Voller-Knoten-Init: Status-Widget (17) → Membran (15) → Siegel (16)
...
(function () {
  ...
  var REPO_URL = "https://github.com/lausiklauskn-png/Kimboard";
  ...
  function boot() {
    ...
    if (window.SbkimWidget && typeof window.SbkimWidget.init === "function") {
      try {
        ...
        console.info("[Kimboard] Status-Widget (Modul 17) gemountet.");
      } catch (e) { console.warn("[Kimboard] Status-Widget übersprungen:", e); }
    }
    ...
    widgetReady.then(function () {
      if (window.SbkimMembrane && typeof window.SbkimMembrane.init === "function") {
        try {
          ...
          console.info("[Kimboard] Membran (Modul 15, Wächter) aktiv.");
        } catch (e) { console.warn("[Kimboard] Membran übersprungen:", e); }
      }
      if (window.SbkimSiegel && typeof window.SbkimSiegel.init === "function") {
        try {
          ...
          console.info("[Kimboard] Siegel (Modul 16) — Selbst-Prüfung läuft (Bronze/Gold).");
        } catch (e) { console.warn("[Kimboard] Siegel übersprungen:", e); }
      }
    });
    ...
    if (window.SbkimApoptose && typeof window.SbkimApoptose.init === "function") {
      ...
      } catch (e) { console.warn("[Kimboard] Apoptose übersprungen:", e); }
    }
  }
