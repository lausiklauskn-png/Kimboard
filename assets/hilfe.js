...
(function () {
  ...
  var TEXTE = {
    ...
    // — Identität / Gerät —
    'me': ['Meine Kennung', 'Deine Absender-Kennung auf dem Brett — daran erkennen dich andere wieder, und damit sind deine Zettel unterschrieben. Sie entsteht beim ersten Öffnen von selbst, ohne Anmeldung. Nicht zu verwechseln mit der SBKIM-Spore fürs Mycel: das ist eine eigene Identität, die erst entsteht, wenn du dich mit dem Netz verbindest. Die Sicherung daneben (🔑) rettet DIESE Kennung.'],
    ...
    'heim-relay':  ['🏠 Heim-Relais', 'Das Relais, auf das deine Zettel GESCHRIEBEN werden. Warum das zählt: Ein Relais speichert, was durch es hindurchgeht. Bei einem fremden Betreiber kannst du später nur BITTEN zu löschen — auf einem eigenen entscheidest du selbst, wer schreiben und lesen darf, und kannst wirklich löschen. Voreingestellt ist relay.family-projekt.de. GELESEN wird weiterhin von allen Relais, die du oben angehakt hast — so entgeht dir nichts, obwohl du schmal sendest.'],
    'heim-nur':    ['Nur dorthin senden', 'Angehakt (Voreinstellung): Deine Zettel gehen NUR auf dein Heim-Relais — nicht an fremde Betreiber. Abgehakt: Sie gehen an alle verbundenen Relais gleichzeitig; das ist ausfallsicherer, verteilt deine Daten aber an Betreiber, bei denen du nichts mehr durchsetzen kannst. Ist dein Heim-Relais gerade nicht erreichbar, wird GEFRAGT, ob es diesmal über fremde gehen soll — es wird nie heimlich umgeschaltet. Ehrlich: Das gilt für deinen Absende-Weg. Was dein Gegenüber mit anderer Einstellung verschickt, landet trotzdem auch anderswo.'],
    'rot-on':     ['🔄 Relais automatisch wechseln', 'Wechselt von selbst zwischen den Relais. Alle im Kreis rechnen aus demselben Geheimnis dieselbe Auswahl aus — ihr müsst euch nichts sagen. NUTZEN: Ausfallsicherheit, und kein einzelnes Relais sieht alles. Es wird nicht weniger als ohne Automatik: die Voreinstellung nimmt genauso viele Relais wie sonst auch. WICHTIG: Das ist KEIN Schutz gegen Sperren oder Überwachung. Beachte auch: Die Automatik STREUT bewusst über viele Relais, das Heim-Relais BÜNDELT bewusst bei einem, dem du traust — beides zugleich geht nicht. Dein Heim-Relais bleibt dabei immer verbunden, damit der Sendeweg nicht abreißt.'],
    ...
    'nachholen':   ['🕓 Älteres nachholen', 'Fragt EINMALIG bei den zuletzt benutzten Relais nach, was du verpasst hast. Wozu: Beim Wechseln der Relais hängt jeder nur an den GERADE gültigen — wer später startet, sieht nie, was vorher über andere Relais lief. Weil sich die Wechsel-Fenster zurückrechnen lassen, weiß deine App, wo sie nachfragen muss. Es läuft einmal und begrenzt, dann ist wieder Ruhe — kein Dauerabruf. Ehrlich: Relais halten nichts garantiert vor. Was ein Relais schon weggeworfen hat, kann auch dieser Knopf nicht zurückholen — Vollständigkeit ist nicht versprochen. Läuft die Automatik, geschieht das einmal beim Start auch von selbst.'],
    ...
    '.kb-mute': ['🔇 Stumm schalten', 'Blendet ALLE Zettel dieses Absenders bei dir aus — dauerhaft, auch nach dem Neuladen. Wirkt NUR auf diesem Gerät: der andere erfährt nichts davon, und niemand sonst sieht deine Liste. Gedacht für den Fall, dass jemand stört oder Unsinn schickt. Rückgängig geht es über „👁 Ausgeblendet" oben am Brett — dort stehen alle stummgeschalteten Absender mit einem Knopf zum Wieder-Zeigen.'],
    '.q-melden': ['⚑ Melden', 'Meldet diese Nachricht dem Betreiber — für Hassrede, Bedrohungen, Betrug und alles andere, was hier nicht hingehört. Warum es diesen Knopf BRAUCHT: Löschen darf nur, wer etwas geschrieben hat. Gegen einen FREMDEN Zettel hilft das nicht, und genau dafür ist das ⚑ da. Ein Mensch sieht sich die Meldung an und entscheidet — es wird nichts automatisch entfernt (sonst könnten drei Meldungen jeden Zettel vom Brett räumen). Du musst deinen Namen nicht angeben. Mitgeschickt werden nur die Kennung des Zettels und dein Text, NICHT der beanstandete Inhalt selbst. Auf Wunsch wird die Nachricht sofort bei dir ausgeblendet — das wirkt nur auf diesem Gerät und unabhängig davon, wie entschieden wird.'],
    '.q-del': ['✕ Löschen', 'Öffnet die Wahl: NUR BEI DIR ausblenden (verschwindet auf diesem Gerät, die anderen sehen die Nachricht weiter) — oder, bei deinen EIGENEN Nachrichten, BEI ALLEN löschen. Letzteres bittet die Relais und die anderen Geräte, sie zu entfernen: Dort erscheint dann kurz der Hinweis „zurückgezogen", danach ist sie weg. Ehrlich dazu: Wir können darum BITTEN — erzwingen können wir es nicht. Wer sie schon gelesen oder abfotografiert hat, hat sie weiterhin. Fremde Nachrichten kannst du nur bei dir ausblenden.'],
    ...
    '.kb-iscontact': ['➖ Kontakt', 'Steht dort, wo vorher „➕ Kontakt" stand: Diese Person IST schon dein Kontakt. Tippen entfernt sie wieder aus deinen Kontakten — nach einer Rückfrage, damit nichts aus Versehen passiert. Ihr könnt euch danach nicht mehr privat schreiben, bis du sie wieder aufnimmst. Das geschieht nur auf diesem Gerät; die andere Person erfährt nichts davon, und ihre bereits empfangenen Nachrichten verschwinden dadurch nicht.'],
    ...
    'dm-groups':   ['👥 Gruppen', 'Eine Gruppe ist eine benannte Runde aus deinen Kontakten — z. B. „Familie". Was du an die Gruppe schreibst, geht als eigene, verschlüsselte Kopie an JEDES Mitglied; Antworten sehen alle, sodass ihr denselben Verlauf lest. Es gibt keinen Gruppen-Server: Wer einlädt, verschickt die Mitgliederliste verschlüsselt, und wer annimmt, hat damit automatisch alle Schlüssel — du musst nichts abtippen. Niemand landet ungefragt in einer Gruppe: Jede Einladung wird erst mit „Annehmen" gespeichert. Ehrlich: Bei N Mitgliedern gehen N Kopien raus — für kleine, private Runden gedacht. Und wer eine Gruppe verlässt, verschwindet nicht rückwirkend aus dem, was die anderen schon haben.'],
    ...
    // — Brett-Ansicht —
    'meine-zurueckziehen': ['🗑 Meine zurückziehen', 'Zieht auf einen Schlag ALLES zurück, was DU hier geschrieben oder geschickt hast — Texte, Bilder, Sprachnachrichten. Die Relais und die anderen Geräte werden gebeten, es zu entfernen; bei dir ist es sofort weg. Fremde Nachrichten bleiben unberührt (sonst könnte jeder das Brett leerräumen). Bei den anderen erscheint kurz der Hinweis „zurückgezogen", danach ist es weg. Ehrlich dazu: Wir können darum BITTEN — erzwingen können wir es nicht. Wer schon gelesen, kopiert oder abfotografiert hat, hat es weiterhin. Einzeln geht dasselbe über das ✕ am jeweiligen Zettel.'],
    'ausgeblendet': ['👁 Ausgeblendet & stumm', 'Der Ort, an dem du alles Weggeklickte wiederfindest: wie viele Nachrichten du ausgeblendet hast, welche Absender du stumm geschaltet hast (mit Knopf zum Wieder-Zeigen) und wie viele Nachrichten zurückgezogen wurden. WICHTIG: Was du ausblendest, wird NICHT gespeichert — wir merken uns nur die Kennung, nie den Text. Darum kannst du dort keine einzelne Nachricht heraussuchen, sondern nur alle zusammen zurückholen — und auch das nur, soweit die Relais sie noch vorrätig haben. Direkt nach dem Ausblenden geht es einfacher: In der Meldung unten steht „↩ Rückgängig".'],
    '.a-del': ['✕ an einer Antwort', 'Blendet NUR diese eine Antwort aus, statt den ganzen Zettel samt allen Antworten. Bei deinen eigenen Antworten steht dort zusätzlich „bei allen löschen". Direkt danach kannst du es mit „↩ Rückgängig" in der Meldung zurücknehmen.'],
    '.kb-undo': ['↩ Rückgängig', 'Holt das gerade Ausgeblendete sofort zurück. Der Knopf steht rund acht Sekunden in der Meldung — danach hilft nur noch „👁 Ausgeblendet" oben am Brett, und dort auch nur alle zusammen.'],
    'board-clear': ['🧹 Leeren', 'Blendet die angezeigten Zettel bei DIR aus (bleibt auch nach dem Neuladen weg). Bei den anderen bleibt alles stehen — es wird nichts wirklich gelöscht. Versehentlich gedrückt? „↩ Rückgängig" in der Meldung holt alle sofort zurück; später geht es über „👁 Ausgeblendet".'],
